            ...
            if (!rst_n) begin
                ...
                case (state)
                    IDLE: begin
                        if (valid_in) begin
                            state <= SENDING;
                            axis_out.tdata <= {{(STREAM_WIDTH - BUFFER_SIZE){1'b0}}, buf_in}; // Pad with zeros if needed
                            axis_out.tkeep <= {{(KEEP_WIDTH - BUFFER_SIZE/8){1'b0}}, {(BUFFER_SIZE/8){1'b1}}}; // Set tkeep for valid bytes
